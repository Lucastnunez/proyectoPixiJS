...
export class GameScene extends SceneBase implements Updateable{
    ...
    private test= new TempText("holaa",5000,this.style);
    ...
    constructor()
    {
        ...
        this.test.position.set(110,50)
        this.test.scale.set(2,2);

        this.hud.addChild(this.test)
    ...
    update(deltaMS:number, _deltaFrame:number){
        ...
        SceneManager.app.renderer.render(this.lightContainer,{renderTexture: this.lightTexture, clear:false});
        ...
        for (const wall of this.background.hitboxes) {
            ...
            if(overlap != null)
            {   
                console.log(this.prota.x,this.prota.y," ",wall.getLocalBounds().x,wall.getLocalBounds().y)
                if(overlap.width<overlap.height){

                    if (this.prota.x > wall.getLocalBounds().x){
                        this.prota.x+= overlap.width;
                        this.prota.speed.x=0
                        console.log("1")
                    }else if (this.prota.x < wall.getLocalBounds().x){
                        this.prota.x-= overlap.width;
                        this.prota.speed.x=0
                        console.log("2")
                    }
                    ...
                    if (this.prota.y > wall.getLocalBounds().y){
                        this.prota.y-= overlap.height;
                        this.prota.speed.y=0
                        console.log("3")
                    }else if (this.prota.y < wall.getLocalBounds().y){
                        this.prota.y+= overlap.height;
                        this.prota.speed.y=0
                        console.log("4")
                    }
